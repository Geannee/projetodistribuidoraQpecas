const Cart = {
    ...
    add(item) {
        ...
        if (exist) {
            if (exist.qty + 1 > maxStock) {
                return {error: true, maxStock};
            }
            exist.qty++;
        } else {
            if (maxStock <= 0) {
                return {error: true, maxStock: 0};
            }
    ...
    updateQty(id, delta) {
        ...
        if (item) {
            ...
            if (delta > 0 && item.qty + delta > maxStock) {
                alert(`⚠️ Desculpe, quantidade máxima em estoque atingida (${maxStock} unid.).`);
                return items;
            }
...
function addToCart(btn) {
    ...
    const rawPlate = btn.getAttribute('data-vehicle') || '';
    const vehicle = rawPlate.replace('·', '-').trim();

    const maxStock = parseInt(btn.getAttribute('data-stock') || '0', 10);

    const currentCartItems = Cart.get();
    const existingItem = currentCartItems.find(i => i.ref === ref);
    const currentQtyInCart = existingItem ? existingItem.qty : 0;

    if (currentQtyInCart + 1 > maxStock) {
        showCartToast(`⚠️ Desculpe, temos apenas ${maxStock} unidade(s) em estoque.`);
    ...
    Cart.add({brand, ref, price, label, icon, name, vehicle, stock: maxStock});
